...
bottlenecks, mutation_probability, half_size_of_populations = [10, 20], [0.01, 0.05, 0.1], [10, 15, 20]
sequence_length = 205
number_of_seeds = 10

def get_opponent_seed_combinations(opponents, number_of_seeds):
    experiment_values = []
...
if __name__ == '__main__':
    ...
    opponents_with_seed = get_opponent_seed_combinations(opponents, number_of_seeds)
    experiments = list(itertools.product(opponents_with_seed, half_size_of_populations,
                                         bottlenecks, mutation_probability))

    for opponent, half_size_of_population, bottleneck, mutation_probability in tqdm(experiments):
