...
def test_get_fitness_of_individual():
    ...
    seed = np.NaN
    i, score_vs_c = ss.get_fitness_of_individual(
        sequence, axl.Cooperator, seed=seed, index=0, turns=10
    ).compute()
    assert i == 0
    assert score_vs_c == 3

    i, score_vs_d = ss.get_fitness_of_individual(
        sequence, axl.Defector, seed=seed, index=0, turns=10
    ).compute()
    assert i == 0
    assert score_vs_d == 0
...
def test_get_fitness_of_population():
    ...
    scores = ss.get_fitness_of_population(
        population, axl.TitForTat, turns=205, seed=0
    )
    assert scores == [
        (0, 1.0439024390243903),
...
def test_get_fitness_of_population_with_random_opponent():
    population = ss.get_initial_population(5)
    scores = ss.get_fitness_of_population(
        population, axl.Random, turns=205, seed=0
    )

    assert scores == [
        (0, 2.8780487804878048),
